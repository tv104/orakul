...
describe("Magic8VRF ", function () {
  async function deployMagic8VRFFixture() {
    ...
    const Magic8VRF = await ethers.getContractFactory("Magic8VRF");
    ...
    // Add Magic8VRF as VRF consumer
    await mockCoordinator.addConsumer(subscriptionId, await magic8VRF.getAddress());

    // Get signers
    const [owner, player] = await ethers.getSigners();

    return { magic8VRF, mockCoordinator, owner, player, subscriptionId };
  }
  ...
  describe("Deployment", function () {
    it("Should deploy successfully", async function () {
      const { magic8VRF } = await loadFixture(deployMagic8VRFFixture);
      expect(await magic8VRF.getAddress()).to.be.properAddress;
    });

    it("Should have correct number of outcomes", async function () {
      const { magic8VRF } = await loadFixture(deployMagic8VRFFixture);
      expect(await magic8VRF.getTotalOutcomes()).to.equal(20);
    });

    it("Should have correct max question length", async function () {
      const { magic8VRF } = await loadFixture(deployMagic8VRFFixture);
      expect(await magic8VRF.getMaxQuestionLength()).to.equal(120);
    });
  });

  describe("Asking a Question", function () {
    it("Should emit PredictionRequested when asking a question", async function () {
      const { magic8VRF, player } = await loadFixture(deployMagic8VRFFixture);
      const question = "will my wife come back if I double my portfolio?";
    
      await expect(magic8VRF.connect(player).askQuestion(question))
        .to.emit(magic8VRF, "PredictionRequested")
    ...
    it("Should store prediction request details correctly", async function () {
      ...
      const tx = await magic8VRF.connect(player).askQuestion(question);
      const requestId = await getRequestId(tx);

      const prediction = await magic8VRF.predictions(requestId);
    ...
    it("Should reject empty questions", async function () {
      ...
      await expect(magic8VRF.connect(player).askQuestion(""))
    ...
    it("Should reject questions that are too long", async function () {
      ...
      const maxLength = await magic8VRF.getMaxQuestionLength();
      const longQuestion = "?".repeat(Number(maxLength) + 1);
      
      await expect(magic8VRF.connect(player).askQuestion(longQuestion))
        .to.be.revertedWith("Question too long");
    });

    it("Should fulfill random request and emit result", async function () {
      const { magic8VRF, mockCoordinator, player } = await loadFixture(deployMagic8VRFFixture);
      ...
      await expect(mockCoordinator.fulfillRandomWordsWithOverride(
        requestId,
        await magic8VRF.getAddress(),
        randomWords
      ))
        .to.emit(magic8VRF, "PredictionResult")
    ...
    it("Should not allow fulfilling same request twice", async function () {
      ...
      const tx = await magic8VRF.connect(player).askQuestion("was that top signal or am I just bad at this?");
      const requestId = await getRequestId(tx);
  ...
  describe("Getting Results", function () {
    it("Should return correct prediction result with question", async function () {
      ...
      const [fulfilled, outcomeIndex, resultPlayer, resultQuestion] = await magic8VRF.getPredictionResult(requestId);
    ...
    it("Should return default values for non-existent prediction", async function () {
      ...
      const [fulfilled, outcomeIndex, player, question] = await magic8VRF.getPredictionResult(999);
